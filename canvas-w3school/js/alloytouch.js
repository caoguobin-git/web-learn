...
(function() {
    ...
    var AlloyTouch = function(option) {
        ...
        this.followers = followersArr.map(function(follower) {
            return {
                element: typeof follower.element === 'string' ? document.querySelector(follower.element) :
                    follower.element,
    ...
    AlloyTouch.prototype = {
        ...
        _end: function(evt) {
            if (this.isTouchStart) {
                ...
                var self = this,
                    ...
                    triggerTap = (Math.abs(evt.changedTouches[0].pageX - this.x1) < 30 && Math.abs(evt.changedTouches[
                        0].pageY - this.y1) < 30);
                if (triggerTap) {
                ...
                if (this.hasMax && current > this.max()) {
                    ...
                    if (dt < 300) {
                        var distance = ((this.vertical ? evt.changedTouches[0].pageY : evt.changedTouches[0]
                                .pageX) - this.start) * this.sensitivity,
                        ...
                        var destination = current + (actualSpeed * actualSpeed) / (2 * this.deceleration) *
                            direction;
                        ...
                        if (destination < this.min()) {
                            if (destination < this.min() - this.maxRegion) {
                                ...
                                tRatio = reverseEase((current - this.min() + this.springMaxRegion * (this.min() -
                                    destination) / this.maxRegion) / (current - destination));
                                destination = this.min() - this.springMaxRegion * (this.min() - destination) /
                                    this.maxRegion;
                            }
                        } else if (destination > this.max()) {
                            if (destination > this.max() + this.maxRegion) {
                                tRatio = reverseEase((this.max() + this.springMaxRegion - current) / (
                                    destination - current));
                                destination = this.max() + this.springMaxRegion;
                            } else {
                                tRatio = reverseEase((this.max() + this.springMaxRegion * (destination -
                                    this.max()) / this.maxRegion - current) / (destination -
                                    current));
                                destination = this.max() + this.springMaxRegion * (destination - this.max()) /
                                    this.maxRegion;
        ...
        _correction: function() {
            ...
            if (Math.abs(dy) > this.step / 2) {
                this._to((value < 0 ? -1 : 1) * (rpt + 1) * this.step, 400, ease, this.change, function(
                    value) {
                    this._calculateIndex();
